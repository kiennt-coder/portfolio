...
const ActivitySummaryBoxItem = ({ icon, name, address, date, description }: ActivitySummaryBoxItemProps) => {
    return (
        <li className="group mb-8 flex flex-row items-start">
            <div>
                <div className="md:size-20 size-11 flex items-center justify-center mt-2 mr-7 text-base-100 dark:text-primary bg-primary dark:bg-transparent border border-primary rounded-lg group-hover:text-primary group-hover:bg-transparent dark:group-hover:text-base-100 dark:group-hover:bg-primary duration-300">
                    {cloneElement(icon as JSX.Element, {
                        className: "md:size-12 size-6 flex-shrink-0 rotate-y-0 group-hover:rotate-y-360 duration-500 ease-out"
                    })}
                </div>
            </div>
            <div>
                <h3 className="xl:text-3xl text-2xl font-semibold mb-2 dark:text-neutral-50">{name}</h3>
